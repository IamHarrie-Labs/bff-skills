...
// Verified on-chain: SM1FKXGNZJWSTWDWXQZJNF7B5TV5ZB235JTCXYXKD.dlmm-swap-router-v-1-1
// Source: https://api.mainnet.hiro.so/v2/contracts/interface/SM1FKXGNZJWSTWDWXQZJNF7B5TV5ZB235JTCXYXKD/dlmm-swap-router-v-1-1
...
async function executeCorrectiveSwap(
  ...
  // Gate: quote staleness
  const quoteAge = Date.now() - quoteTime;
  if (quoteAge > MAX_QUOTE_AGE_MS) {
    throw new Error(
      `QUOTE_STALE: Quote is ${Math.round(quoteAge / 1000)} s old — exceeds 45-second freshness gate. Re-run scan.`,
    );
  }
  ...
  // ── Post-conditions for PostConditionMode.Deny ──────────────────────────
  // Explicitly allow only the tokens that move during this swap.
  // Pc builder API (v7+): Pc.principal(addr).willSendEq|Gte(amount).ustx()|ft(contract, assetName)
  const postConditions: any[] = [];

  const fromToken = isXtoY ? pool.token_x : pool.token_y;
  const fromSymbol = isXtoY ? pool.token_x_symbol : pool.token_y_symbol;
  const toToken = isXtoY ? pool.token_y : pool.token_x;
  const toSymbol = isXtoY ? pool.token_y_symbol : pool.token_x_symbol;

  // 1. Sender sends exactly amount_in of the from-token
  if (fromSymbol.toUpperCase() === "STX") {
    postConditions.push(
      (Pc as any).principal(walletAddress).willSendEq(BigInt(plan.amount_in)).ustx(),
    );
  } else {
    const fAssetName = fromToken.split(".")[1] ?? fromToken;
    postConditions.push(
      (Pc as any).principal(walletAddress).willSendEq(BigInt(plan.amount_in)).ft(fromToken, fAssetName),
    );
  }

  // 2. Pool contract sends at least minimum_out of the to-token to the sender
  const poolPrincipal = pool.pool_contract; // e.g. "SM...ADDR.dlmm-pool-..."
  if (toSymbol.toUpperCase() === "STX") {
    postConditions.push(
      (Pc as any).principal(poolPrincipal).willSendGte(BigInt(plan.minimum_out)).ustx(),
    );
  } else {
    const tAssetName = toToken.split(".")[1] ?? toToken;
    postConditions.push(
      (Pc as any).principal(poolPrincipal).willSendGte(BigInt(plan.minimum_out)).ft(toToken, tAssetName),
    );
  }

  // ── Build swap call via DLMM swap router ──────────────────────────────
  // Verified contract: SM1FKXGNZJWSTWDWXQZJNF7B5TV5ZB235JTCXYXKD.dlmm-swap-router-v-1-1
  // Function signatures:
  //   swap-x-for-y-same-multi(swaps: list, x-token-trait, y-token-trait, amount, min-y-amount-total, max-unfavorable-bins)
  //   swap-y-for-x-same-multi(swaps: list, x-token-trait, y-token-trait, amount, min-x-amount-total, max-unfavorable-bins)
  const [poolAddr, poolName] = pool.pool_contract.split(".");
  const [xAddr, xName] = pool.token_x.split(".");
  const [yAddr, yName] = pool.token_y.split(".");

  // Single-entry swap list targeting the active bin
  const swapEntry = tupleCV({
    "expected-bin-id": intCV(pool.active_bin),
    "min-received":    uintCV(0n),           // rely on min-y-amount-total for total protection
    "pool-trait":      contractPrincipalCV(poolAddr, poolName),
  });

  const fnName       = isXtoY ? "swap-x-for-y-same-multi" : "swap-y-for-x-same-multi";
  const minTotalArg  = isXtoY ? "min-y-amount-total" : "min-x-amount-total";
  ...
  const tx = await makeContractCall({
    ...
    functionArgs: [
      ...
      uintCV(BigInt(plan.amount_in)),
      uintCV(BigInt(plan.minimum_out)),   // min-y-amount-total / min-x-amount-total
      uintCV(5n),                          // max-unfavorable-bins: allow up to 5 adjacent bins
    ],
