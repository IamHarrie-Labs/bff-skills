...
- Execute the returned `mcpCommand` via `zest_repay` after user confirmation (or autonomously if configured).
...
- Map error codes to user guidance:
  ...
  - `insufficient_balance` → "Wallet balance too low after reserve. Deposit more tokens."
  - `no_collateral` → "Supply collateral before borrowing."
  - `cooldown_active` → "Wait for cooldown, or use repay (cooldown-exempt)."
...
## Lifecycle example (supply → borrow → repay)

```
1. doctor            → confirm wallet + gas
2. status            → read positions (empty)
3. supply sBTC 10000 → supply collateral (HF: ∞)
4. status            → confirm supply registered
5. borrow USDC 4000  → borrow stablecoin (HF: ~1.8)
6. status            → confirm new HF, available capacity
7. [time passes, market moves, HF drops to 1.35]
8. manage --target-hf=1.6 → compute minimum repay
9. repay USDC 1500   → restore HF to 1.6
10. status           → confirm HF recovered
```
